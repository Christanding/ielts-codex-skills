...
name: ielts-reading
description: |
  雅思阅读精读教练。同义替换提取 + T/F/NG逻辑拆解 + 段落结构分析 + 错题诊断。V1.0 起每次分析写带 frontmatter 的 submission，同义替换拆成 yaml 子文件。
...
每次分析完成后，写**两个文件**（不再写 index/errors）：
...
- 闭环 issue_key 使用 `reading:{errors[].type || unknown}:{errors[].tag}`；`question` 只作为 evidence，不能进入主键
...
- 精读训练不直接给答案——引导式教学
- 你不直接写 `coach_notes.md`；如果同类错因反复出现，只建议用户让 `/ielts` 或 `/ielts-diagnose` 记录
